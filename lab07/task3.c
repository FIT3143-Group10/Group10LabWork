...
int main(int argc, char **argv) {
    ...
    // Receive a row from the lower rank process and this is the ghost points in
    // the first row
    if (rank == 0) { // Rank 0 process only needs to send
        MPI_Send(xlocal[maxn / size], maxn, MPI_DOUBLE, rank + 1, 0, MPI_COMM_WORLD);
    } else if (rank == size - 1) { // Highest rank process only needs to receive
        MPI_Recv(xlocal[0], maxn, MPI_DOUBLE, rank - 1, 0, MPI_COMM_WORLD, &status);
    } else { // All other processes need to send AND receive
        MPI_Sendrecv(xlocal[maxn / size], maxn, MPI_DOUBLE, rank + 1, 0,
                     xlocal[0], maxn, MPI_DOUBLE, rank - 1, 0, MPI_COMM_WORLD, &status);
    }

    // Send the first row of the middle rows to lower rank process
    // Receive a row from the upper tank process and this is the ghost points in
    // the last row
    if (rank == 0) {
        MPI_Recv(xlocal[maxn / size + 1], maxn, MPI_DOUBLE, rank + 1, 0, MPI_COMM_WORLD, &status);
    } else if (rank == size - 1) {
        MPI_Send(xlocal[1], maxn, MPI_DOUBLE, rank - 1, 0, MPI_COMM_WORLD);
    } else {
        MPI_Sendrecv(xlocal[1], maxn, MPI_DOUBLE, rank -1, 0,
                     xlocal[maxn / size + 1], maxn, MPI_DOUBLE, rank + 1, 0, MPI_COMM_WORLD, &status);
    }
    ...
    // Write xlocal of each process into a unique text file (the rank is
    // appended into the text file name)
